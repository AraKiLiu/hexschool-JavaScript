                  ...
                  <div class="input-group">
                    <div class="input-group-prepend">
                      <button class="btn btn-outline-primary"
                        type="button"
                        @click="item.quantity++; updataQuantity(item.product.id, item.quantity)">
                        +
                      </button>
                    </div>
                    <input id="inlineFormInputGroupUsername" type="number" min="1"
                     class="form-control text-center" V-model="item.quantity"
                      @change="updataQuantity(item.product.id, item.quantity)">
...
export default {
  ...
  created() {
    this.$http.get(`${process.env.VUE_APP_APIPATH}api/${process.env.VUE_APP_UUID}/ec/products`).then((response) => {
